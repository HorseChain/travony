...
function getDeploymentDomain() {
  ...
  // For production deployment, use the internal app domain (no port needed)
  if (process.env.REPLIT_INTERNAL_APP_DOMAIN) {
    return stripProtocol(process.env.REPLIT_INTERNAL_APP_DOMAIN);
  }

  // For development, use port 5000 so Expo Go connects to Express server (not Metro)
  if (process.env.REPLIT_DEV_DOMAIN) {
    const domain = stripProtocol(process.env.REPLIT_DEV_DOMAIN);
    // Add port 5000 for development to route to Express server serving static builds
    return domain.includes(":") ? domain : `${domain}:5000`;
  }
...
function updateManifests(manifests, timestamp, baseUrl, assetsByHash) {
  const updateForPlatform = (platform, manifest) => {
    ...
    // Fix icon URLs to use correct base URL with port
    const expoClient = manifest.extra.expoClient;
    if (expoClient.iconUrl) {
      expoClient.iconUrl = `${baseUrl}/assets/images/icon.png`;
    }
    if (expoClient.android?.adaptiveIcon) {
      const ai = expoClient.android.adaptiveIcon;
