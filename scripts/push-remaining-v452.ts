...
async function main() {
  console.log('=== Pushing remaining v4.5.2 files ===\n');
  ...
  // Get all files and filter to those that likely weren't pushed (files from index 121+)
  const allFiles = [
  ...
  // Push files from index 121 onwards (the ones that timed out)
  const remaining = allFiles.slice(121);
  console.log(`Pushing ${remaining.length} remaining files...\n`);

  let ok = 0, fail = 0;
  for (let i = 0; i < remaining.length; i++) {
    const f = remaining[i];
    const success = await pushFile(octokit, owner, repoName, branch, f, baseDir, `v4.5.2 Hub intelligence + live feed: ${f}`);
    if (success) { ok++; process.stdout.write(`\r  [${i+1}/${remaining.length}] ${ok} OK`); }
    else fail++;
  ...
  try {
    ...
    if (bw) {
      ...
      console.log('No build workflow found. Available:');
      for (const w of workflows.workflows) console.log(`  - ${w.name} (${w.id})`);
    }
  } catch (e: any) {
