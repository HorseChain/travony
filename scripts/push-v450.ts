...
function collectFiles(baseDir: string, dir: string, extensions: string[]): string[] {
  ...
  const walk = (d: string) => {
    for (const entry of fs.readdirSync(d, { withFileTypes: true })) {
      if (['node_modules', '.expo', 'static-build', '.git'].includes(entry.name)) continue;
...
async function main() {
  console.log('=== Pushing Travony v4.5.0 Full Source to GitHub ===\n');
  ...
  const allFiles = [
    ...
    ...collectFiles(baseDir, 'scripts', ['.ts', '.js', '.sh', '.mjs']),
  ];
  ...
  for (let i = 0; i < allFiles.length; i++) {
    const f = allFiles[i];
    const success = await pushFile(octokit, owner, repoName, branch, f, baseDir, `v4.5.0: ${f}`);
