...
async function pushFile(octokit: Octokit, owner: string, repo: string, branch: string, filePath: string, baseDir: string, commitMsg: string): Promise<boolean> {
  ...
  try {
    ...
    console.log(`  FAIL ${filePath}: ${e.status || e.message}`);
    return false;
  }
}
...
async function main() {
  console.log('=== Push Remaining + Trigger Build ===\n');
  const accessToken = await getAccessToken();
  const octokit = new Octokit({ auth: accessToken });
  const owner = 'HorseChain', repoName = 'travony', branch = 'main';
  ...
  console.log(`Checking ${allFiles.length} files against GitHub...\n`);
  ...
  for (let i = 0; i < allFiles.length; i++) {
    ...
    const localContent = fs.readFileSync(fullPath).toString('base64');
    
    let remoteSha: string | undefined;
    let remoteContent: string | undefined;
    try {
      const { data } = await octokit.rest.repos.getContent({ owner, repo: repoName, path: f, ref: branch });
      if (!Array.isArray(data) && 'sha' in data) {
        remoteSha = data.sha;
        remoteContent = (data as any).content?.replace(/\n/g, '');
      }
    } catch {}
    ...
    const success = await pushFile(octokit, owner, repoName, branch, f, baseDir, `v4.5.1: ${f}`);
    if (success) {
      pushed++;
      process.stdout.write(`\r  Pushed ${pushed} files...`);
    } else {
      failed++;
    }
  ...
  console.log(`\n\nResult: ${pushed} pushed, ${skipped} up-to-date, ${failed} failed\n`);
  ...
  try {
    ...
    if (bw) {
      ...
      console.log(`Build "${bw.name}" triggered!`);
    } else {
      console.log('No build workflow found');
    }
  } catch (e: any) {
